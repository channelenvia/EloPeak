...
describe('Database authorization migration', () => {
  ...
  it('nao permite que as telas gravem diretamente em order_messages', async () => {
    ...
    // O chat agora é sempre renderizado via OrderDetailShell (layout
    // compartilhado pelas 3 páginas -- ver refactor estrutural de
    // OrderDetail/JobDetail), então a garantia "sempre usa <OrderChat, nunca
    // acessa order_messages direto" é verificada na página (chama o shell) +
    // no shell (que de fato renderiza o componente seguro).
    const shell = await readFile(new URL('../src/components/order/OrderDetailShell.tsx', import.meta.url), 'utf8')
    expect(shell).toContain('<OrderChat')
    expect(shell).not.toContain("from('order_messages')")

    for (const page of pages) {
      expect(page).toContain('<OrderDetailShell')
