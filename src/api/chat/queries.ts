...
import type { OrderChatMentionTarget, OrderChatState } from './types'
...
export async function getOrderChatMentionTargets(orderId: string): Promise<OrderChatMentionTarget[]> {
  const { data, error } = await supabase.rpc('get_order_chat_mention_targets', { p_order_id: orderId })
  if (error) throw normalizeApiError(error)
  const result = data as unknown as { success: boolean; code?: string; message?: string; targets?: OrderChatMentionTarget[] }
  if (!result.success) {
    throw new ApiError(CHAT_ERROR_MESSAGES[result.code ?? ''] ?? result.message ?? 'Não foi possível carregar quem pode ser mencionado.', { code: result.code })
  }
