...
export async function sendOrderMessage(params: { orderId: string; content: string; mentionedUserIds?: string[] }) {
  const { data, error } = await supabase.rpc('send_order_message', {
    p_order_id: params.orderId, p_content: params.content, p_mentioned_user_ids: params.mentionedUserIds ?? [],
  })
