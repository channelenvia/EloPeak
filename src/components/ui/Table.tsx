  ...
  return (
    <tr
      className={cn(
        ...
        clickable && 'cursor-pointer hover:bg-bg-interactive',
        className
      )}
      {...props}
    />
...
export function TableHead({ className, ...props }: React.ThHTMLAttributes<HTMLTableCellElement>) {
  return (
    <th
      className={cn('py-2.5 px-4 text-left text-xs font-semibold uppercase tracking-wide text-ink-muted', className)}
      {...props}
    />
  )
}
...
export function TableCell({ className, ...props }: React.TdHTMLAttributes<HTMLTableCellElement>) {
  return <td className={cn('py-2.5 px-4 text-ink-secondary tabular-figures', className)} {...props} />
}
