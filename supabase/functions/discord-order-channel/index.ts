...
import {
  ...
  fetchOrderProfiles, buildPublicJobEmbed, buildExclusiveJobDM, sendChannelMessage, sendDirectMessage,
...
serve(async (req) => {
  ...
  try {
    ...
    if (newStatus === 'awaiting_assignment' && oldStatus !== 'awaiting_assignment') {
      ...
      // Zera antes de tentar anunciar: se este pedido já passou por
      ...
      await supabaseAdmin().from('orders').update({ awaiting_assignment_announced_at: new Date().toISOString() }).eq('id', orderId)
