...
serve(async (req) => {
  ...
  try {
    ...
    let sent: number
    try {
      sent = await notifyAdmins(dm, 'discord-admin-review-alert')
    } catch (err) {
      console.error('discord-admin-review-alert: admin lookup failed', err instanceof Error ? err.message : err)
