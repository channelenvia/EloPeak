-- Bug (HIGH): apply_order_drop nunca valida o status atual do pedido contra
-- o p_from_status que o chamador afirma -- só checa assigned_booster_id is
-- not null. admin_drop_order e admin_reassign_booster sempre passam o
-- v_order.status lido sob lock imediatamente antes da chamada (correto por
-- construção), mas resolve_drop_request chama
-- apply_order_drop(v_req.order_id, 'drop_requested', ...) sem nunca
-- reconfirmar que orders.status ainda é 'drop_requested' -- um admin que use
-- update_order_status/admin_override_order_status pra mover o pedido
-- enquanto um drop request está pendente permite que um approve posterior
-- atropele o que quer que o pedido tenha virado.
--
-- Fix: adicionar a validação uma única vez dentro do próprio
-- apply_order_drop (o ponto por onde os três chamadores passam), em vez de
-- duplicar a checagem em cada chamador.
--
-- Também inclui duas limpezas de eficiência no mesmo branch Mestre+
-- (MEDIUM/LOW, mesma função, sem mudança de comportamento):
-- - fetched_tier/fetched_division/fetched_lp da última order_rank_verifications
--   viram uma única query em vez de duas idênticas exceto pela coluna.
-- - o lock de booster_profiles (antes um `perform 1 ... for update` seguido
--   de um select separado pra is_top3) vira um único select ... for update.
