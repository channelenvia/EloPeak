-- Cleanup (não-funcional): o cálculo de penalidade negativa em
-- apply_order_drop -- round((requester=='booster' ? full : share) *
-- losses_played, 2) -- estava duplicado em 3 lugares (win_boost/md5,
-- elo_boost Mestre+, elo_boost padrão). Foi esse tipo de cópia manual que
-- deixou win_boost/md5 desatualizado em relação a elo_boost até a migration
-- anterior (20260908000000). Extrai a fórmula pra uma função só.
--
-- Resultado numérico idêntico ao anterior em todos os 3 branches -- no
-- branch elo_boost padrão, os dois lados do case (full/4 e share/4) agora
-- são arredondados antes da chamada em vez de depois do case, mas como o
-- case só escolhe um dos dois já-arredondados, o valor final não muda.
create or replace function public.compute_drop_penalty(
  p_requester_role public.drop_requester_role,
  p_full_value     numeric,
  p_share_value    numeric,
  p_losses_played  integer
) returns numeric
language sql
immutable
as $$
...
revoke all on function public.compute_drop_penalty(public.drop_requester_role, numeric, numeric, integer) from public, anon;
grant execute on function public.compute_drop_penalty(public.drop_requester_role, numeric, numeric, integer) to authenticated, service_role;
...
begin
  ...
  -- ── Elo/Duo Boost -- padrão (abaixo de Mestre) ──────────────────────────
    ...
    if v_is_positive then
      ...
      -- Cada lado do case é arredondado por quarto antes de escolher, pra
      -- preservar o double-rounding do cálculo original (round(pick/4,2)
      -- seguido de round(_ * losses, 2) -- ver comentário no topo desta
      -- migration).
      v_penalty := public.compute_drop_penalty(
        p_requester_role,
