-- Bug (CRITICAL, financeiro): no branch win_boost/md5 de apply_order_drop, a
-- penalidade negativa é um corte FIXO de v_order.total_price (booster paga o
-- preço cheio, admin/cliente descontam só o share) -- ao contrário dos
-- branches elo_boost (Mestre+ e padrão), que escalam a penalidade por
-- v_order.losses_played. Bug antigo, existe desde a formulação original em
-- 20260903150200_drop_value_formulas.sql, não é regressão de nenhum fix
-- recente.
--
-- Efeito prático: a cada drop, v_new_wins_purchased infla o total_price pra
-- cobrir as vitórias extras exigidas pelas derrotas do booster que acabou de
-- sair (purchased - wins_played + losses_played). Quando o pedido reabre pro
-- próximo booster, ele já herda esse total_price maior. Como a penalidade de
-- win_boost é uma fração FIXA desse total_price (não escalada pelas próprias
-- derrotas dele), esse próximo booster pode dropar depois de perder só 1
-- partida na própria janela (wins_played/losses_played corretamente
-- zerados/isolados por 20260906240000) e ainda assim pagar uma penalidade
-- dimensionada pelo preço já inflado pelas derrotas do booster anterior --
-- na prática, o valor de várias derrotas alheias, não da sua única derrota.
--
-- Fix: escalar a penalidade de win_boost/md5 por v_order.losses_played,
-- igual aos branches elo_boost. Único trecho alterado; resto da função é
-- idêntico à versão vigente (20260906080000).
create or replace function public.apply_order_drop(p_order_id uuid, p_from_status text, p_actor_id uuid, p_reason text, p_requester_role drop_requester_role)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if v_order.service_type in ('win_boost', 'md5') then
    ...
    if v_is_positive then
      ...
      -- Escalado por losses_played (igual elo_boost) em vez de um corte fixo
      -- de total_price -- ver comentário no topo da migration.
      v_penalty := round(
        (case when p_requester_role = 'booster' then v_win_value_unit else round(v_win_value_unit * v_share_pct, 2) end)
