-- Coaching não deve disputar os slots normais (3 booster comum / 4 Top3) nem
-- o slot exclusivo -- booster pode aceitar quantos pedidos de coaching
-- quiser, em paralelo com os slots normais/exclusivo já em uso.
--
-- Antes desta migration, booster_active_slot_counts contava QUALQUER
-- service_type (inclusive coaching) contra o limite de 3/4, então um booster
-- com 3 coachings ativos ficava travado pra aceitar pedidos normais -- e o
-- contrário também: 3 pedidos normais bloqueavam a aceitação de coaching.

-- 1) Contagem de slots normais passa a ignorar coaching por completo.
...
-- 2) can_booster_accept_order ganha p_service_type: quando for 'coaching',
--    libera sempre (booster aprovado já basta), sem checar total_count/max_total.
--    Default null preserva a assinatura pra chamadas existentes (ex.: consulta
--    de status geral dos slots, que não se refere a um pedido específico).
...
-- 3) accept_boost_order passa o service_type do pedido pra
--    can_booster_accept_order, pra que coaching entre na regra acima.
...
begin
  ...
  v_check := public.can_booster_accept_order(p_booster_user_id, v_order.boost_mode, v_order.service_type);
