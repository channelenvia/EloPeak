-- Bug (ALTO, achado em auditoria): apply_order_drop paga o booster pelo
-- progresso "entregue" (wins_played / rank steps cruzados) sem travar isso
-- no que foi CONTRATADO no pedido. Pedidos ficam em 'drop_requested' até um
-- admin resolver (resolve_drop_request), e o cron de sync de partidas
-- (cron-sync-order-matches) continua sincronizando resultado normalmente
-- nesse status -- então um booster pode continuar ganhando partidas (ou
-- subindo de rank) DEPOIS do drop já ter sido solicitado mas ANTES de ser
-- processado. Quando finalmente processa:
--   - win_boost/md5: v_payout usa wins_played cru, sem min() com
--     wins_purchased -- 6 vitórias sincronizadas num pedido de 3 paga o
--     dobro do valor máximo possível do pedido.
--   - elo_boost abaixo de Mestre: v_steps_crossed vem só de
--     order_rank_verifications, sem teto em v_divisions_remaining -- uma
--     conta que sobe além do rank alvo contratado paga por divisões que
--     nunca foram vendidas.
-- (O branch Mestre+ já trava v_quarters_completed em `least(4, ...)` --
-- não precisa de mudança.)
--
-- Fix: least() no progresso usado pra calcular o payout, nos dois branches,
-- travando no que foi de fato contratado (wins_purchased / divisions
-- remaining do pedido). Resto da função idêntico à versão vigente
-- (migrations_archive/20260908090000).
--
-- Bug secundário (MÉDIO, mesma auditoria): o branch de penalidade credita
-- o valor em booster_ledger_entries mas nunca debita
-- booster_profiles.total_earnings -- assimétrico com o branch de payout
-- logo acima, que sempre soma em total_earnings. O saldo sacável real
-- (booster_available_balance) já é derivado do ledger, então dinheiro não
-- corre risco -- é só o "total ganho" exibido ao admin que ficava inflado
-- depois de qualquer drop com penalidade. Fix: mesmo padrão do payout,
-- espelhado pro lado negativo.
