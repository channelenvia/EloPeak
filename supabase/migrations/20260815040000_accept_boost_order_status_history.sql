-- accept_boost_order muda orders.status pra 'assigned' mas nunca gravava em
-- order_status_history (diferente de update_order_status/admin_override_
-- order_status/request_order_drop/resolve_drop_request, que sempre logam).
-- Resultado: a timeline do pedido (OrderTimeline.tsx) pulava direto de
-- "Esperando Booster" pra qualquer status seguinte, sem nunca mostrar o
-- momento em que o booster pegou o pedido. Replica o mesmo padrão de insert
-- usado nas outras RPCs de transição de status.
