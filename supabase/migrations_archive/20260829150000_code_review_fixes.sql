-- Corrige 3 achados do /code-review sobre as migrations 20260829120000 e
-- 20260829140000 (ambas já aplicadas em produção -- por isso uma migration
-- nova em vez de editar as anteriores).

-- ═══════════════════════════════════════════════════════════════════════
-- 1. refresh_booster_performance_segments -- o DISTINCT ON introduzido pra
--    corrigir o duplicate-key (20260829120000) resolve o caso onde as duas
--    linhas colidentes são IDÊNTICAS em valor (coaching sempre com
--    rank_bucket '__all__', então "com quebra por rank" e "sem quebra por
--    rank" cobrem exatamente a mesma partição). Mas quando um booster tem
--    partidas com queue_type/rank real E partidas com queue_type/rank nulo
--    de origens DIFERENTES, as duas linhas colidem na mesma chave
--    normalizada mas têm valores DIFERENTES (uma é o total geral, a outra
--    é só a fatia com dimensão nula) -- DISTINCT ON escolhe uma
--    arbitrariamente e descarta dados reais, silenciosamente, o que é pior
--    que o crash anterior.
--
--    Fix de raiz: a colisão só existe porque uma linha "sem quebrar por X"
--    (X nulo por não fazer parte do grouping) fica indistinguível de uma
--    linha "quebrando por X" quando o valor real de X é nulo. Filtra as
--    partidas com X nulo PRA FORA dos grouping sets que quebram por X --
--    dessa forma, "quebrado por X" sempre normaliza pra um valor real
--    (nunca '__all__') e "não quebrado por X" sempre normaliza pra
--    '__all__' (nunca um valor real) -- as duas nunca mais colidem, pra
--    NENHUMA combinação de booster/partidas. Com isso o DISTINCT ON deixa
--    de ser necessário (nenhuma colisão é possível de novo) -- removido,
--    pra que uma colisão inesperada no futuro volte a estourar alto (como
--    antes) em vez de silenciosamente esconder dado errado.
-- ═══════════════════════════════════════════════════════════════════════
...
begin
  ...
  match_stats as (
    -- Sets sem quebra por rank_bucket/queue_type -- incluem TODAS as
    -- partidas (rank/queue nulos inclusive), já que não tentam quebrar por
    -- essas dimensões.
    select
    ...
    -- Sets com quebra por rank_bucket -- só partidas com rank_bucket real
    -- (current_rank reconhecido); sem isso, uma partida sem rank normaliza
    -- pra '__all__' igual a uma linha "sem quebrar por rank" e colide.
    select
    ...
    from match_source ms
    where (p_booster_id is null or ms.assigned_booster_id = p_booster_id)
      and ms.current_rank->>'tier' is not null
    group by grouping sets (
    ...
    -- Set com quebra por queue_type -- só partidas com queue_type real.
    select
  ...
  review_stats as (
    ...
    where r.is_public = true and r.booster_id is not null
      and (p_booster_id is null or r.booster_id = p_booster_id)
      and o.current_rank->>'tier' is not null
    group by grouping sets (
...
-- ═══════════════════════════════════════════════════════════════════════
-- 2. apply_order_drop -- v_customer_paid só lia a linha 'paid' mais
--    recente; troca pra somar TODAS as linhas 'paid' do pedido, pro caso
--    (hoje improvável, mas não impossível) de existir mais de uma.
-- ═══════════════════════════════════════════════════════════════════════
...
begin
  ...
  if v_negative_matches > 0 and v_order.service_type in ('elo_boost', 'win_boost', 'md5') then
    ...
    -- Soma TODAS as linhas pagas do pedido (não só a mais recente) -- o
    -- resultado é o mesmo hoje (normalmente 1 linha 'paid' por pedido), mas
    -- nunca subestima o que o cliente pagou se algum dia existir mais de
    -- uma (ex.: reentrega duplicada de webhook criando uma segunda linha).
...
-- ═══════════════════════════════════════════════════════════════════════
-- 3. elo_div_price_cents -- faltava o revoke que win_penalty_price_cents
--    (mesmo padrão: RLS ligada, sem policy, só SECURITY DEFINER acessa) já
--    tem desde 20260829040000.
-- ═══════════════════════════════════════════════════════════════════════
revoke all on public.elo_div_price_cents from public, anon, authenticated;
