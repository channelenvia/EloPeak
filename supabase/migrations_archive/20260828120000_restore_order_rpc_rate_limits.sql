-- Mesma causa raiz da migration anterior nesta sessão (update_order_status):
-- accept_boost_order, set_order_credentials, get_order_credentials,
-- request_order_drop e request_customer_order_drop já tinham
-- check_own_write_rate_limit adicionado numa migration anterior, mas o
-- arquivo rastreado atual desse conjunto de funções não incorpora essa
-- proteção -- nenhuma delas tem edge function na frente (chamadas direto
-- via supabase.rpc() do client), então sem isso ficam sem throttle nenhum,
-- inclusive get/set_order_credentials, que fazem pgp_sym_encrypt/decrypt
-- (custo de CPU por chamada).
